...
def rotation_3d(omega, theta):
    ...
    skew = skew_3d(omega)
    return np.identity(3) + np.sin(norm*theta)*skew/np.linalg.norm(omega) + \
            (1-np.cos(norm*theta))*(skew/norm)**2
    return rot
...
def hat_2d(xi):
    ...
    #YOUR CODE HERE
    return np.array([0, -xi[2], xi[0], xi[2], 0, xi[1], 0,0,0]).reshape(3,3)
    return xi_hat
...
def homog_2d(xi, theta):
    ...
    p = (np.identity(2) - R).dot(np.matrix([[0, -1],[1,0]])).dot(np.matrix(
        [[xi[0]/xi[2]], [xi[1]/xi[2]]]))

    x1 = np.array([0,0])
    t1 = np.vstack((R, x1))
    t2 = np.vstack((p, np.array([1])))
    g = np.hstack((t1, t2))
    return g
...
def homog_3d(xi, theta):
    ...
    v = xi[:3].T
    t1 = 1.0/np.linalg.norm(w)**2.0
    a1 = (np.identity(3) - R)
    a2 = skew_3d(w).dot(v)
    w = np.matrix(w).T
    v = np.matrix(v).T
    t2 = a1.dot(a2.T)
    t3 = w.dot(w.T).dot(v)*theta 
      
    x1 = t1 * (t2 + t3)
    q1 = np.vstack((R, np.array([0.,0.,0.])))
    q2 = np.vstack((x1, np.array([1.])))
    g = np.hstack((q1, q2))
...
def prod_exp(xi, theta):
    ...
    #YOUR CODE HERE
    g = homog_3d(np.array([0,0,0,1,2,3]), 0)
    for subXi, subTheta in zip(xi.T, theta):
        g = g.dot(homog_3d(subXi, subTheta))
